...
namespace Comet
{
    ...
    {
        public static MauiAppBuilder EnableHotReload(
            this MauiAppBuilder builder,
            string? ideIp = null,
            int idePort = Constants.DEFAULT_PORT
        )
        {
        ...
        class HotReloadBuilder : IMauiInitializeService
        {
            ...
            public async void Initialize(IServiceProvider services)
            {
                ...
                await Task.Run(async () =>
                {
                    try
                    {
                        var success = await Reloadify.Reload.Init(
                            IdeIp,
                            IdePort
                        );

                        Console.WriteLine(
                            $"HotReload Initialize: {success}"
                        );
                    }
                    catch (Exception ex)
                    {
